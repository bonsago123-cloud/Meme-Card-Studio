...
export const DEFAULT = {
  ...
  rotation: 0,
  fontFamily: 'studio',
  stretchToRatio: false,
};

// 예전 제출본의 필수 상태는 그대로 요구한다. 새 필드는 이전 템플릿 호환을 위해 기본값을 채운다.
const REQUIRED_LEGACY_KEYS = ['text','ratio','size','color','background','x','y','align','shadow','fit','image','art'];

export const PRESETS = [
  { id: 'pace', name: '나의 속도로', description: '차분한 응원 · 1:1', state: { ...DEFAULT } },
  { id: 'night', name: '밤의 문장', description: '생각을 담는 순간 · 4:5', state: { ...DEFAULT, text: '깊어지는 밤,\n선명해지는 생각.', ratio: '4:5', background: '#36326b', art: 'night', size: 72 } },
  { id: 'start', name: '작은 시작', description: '싱그러운 한마디 · 9:16', state: { ...DEFAULT, text: '작은 시작이\n큰 변화를.', ratio: '9:16', background: '#e0eb9c', color: '#414b33', art: 'lime', shadow: false } },
];
...
export function validateState(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw Error('필수 편집 항목이 없거나 올바르지 않습니다.');
  for (const key of REQUIRED_LEGACY_KEYS) {
    if (!Object.prototype.hasOwnProperty.call(input, key)) throw Error('필수 편집 항목이 없거나 올바르지 않습니다.');
  }
  ...
  if (
    ...
    typeof s.stretchToRatio !== 'boolean' ||
  ...
  for (const [k, min, max] of [
    ['size', 18, 160], ['x', 5, 95], ['y', 5, 95], ['rotation', -180, 180],
    ['imageScale', 25, 400], ['imageX', -50, 150], ['imageY', -50, 150],
    ['cropTop', 0, 45], ['cropRight', 0, 45], ['cropBottom', 0, 45], ['cropLeft', 0, 45],
  ]) {
    if (typeof s[k] !== 'number' || !Number.isFinite(s[k]) || s[k] < min || s[k] > max) {
      throw Error('크기 또는 위치가 허용 범위를 벗어났습니다.');
    }
  }
  if (s.cropTop + s.cropBottom >= 90 || s.cropLeft + s.cropRight >= 90) throw Error('사진을 너무 많이 잘랐습니다. 자르기 값을 줄여주세요.');
...
export function parseTemplates(raw) {
  let data;
  try { data = JSON.parse(raw); } catch { throw Error('JSON 문법이 손상되어 가져오지 않았습니다.'); }
...
export function layoutText(ctx, s, w, h) {
  const margin=54, width=w-margin*2;
  let size=s.size, lines;
  ...
  const lineHeight=size*1.35, height=lines.length*lineHeight;
...
export function imageSourceRect(s,img){
  const sx=img.width*(s.cropLeft/100), sy=img.height*(s.cropTop/100);
  const sw=img.width*(1-(s.cropLeft+s.cropRight)/100), sh=img.height*(1-(s.cropTop+s.cropBottom)/100);
  return {sx,sy,sw,sh};
}

export function imageDestinationRect(s,img,w,h){
  const {sw,sh}=imageSourceRect(s,img);
  const scaleFactor=s.imageScale/100;
  let drawWidth,drawHeight;
  if(s.stretchToRatio){
    // 비율 맞춤 모드: 선택된 화면비 전체에 사진을 비례 유지 없이 늘이거나 압축한다.
    // 따라서 화면비 버튼을 눌러도 자동 잘림 없이 사진 전체(수동 크롭 이후 영역)가 보인다.
    drawWidth=w*scaleFactor;
    drawHeight=h*scaleFactor;
  }else{
    const baseScale=s.fit==='cover'?Math.max(w/sw,h/sh):Math.min(w/sw,h/sh);
    drawWidth=sw*baseScale*scaleFactor;
    drawHeight=sh*baseScale*scaleFactor;
  }
  const centerX=s.imageX/100*w,centerY=s.imageY/100*h;
  return {x:centerX-drawWidth/2,y:centerY-drawHeight/2,width:drawWidth,height:drawHeight,centerX,centerY};
}
...
export function render(ctx,s,img){
  ...
  // 사진을 올린 동안에는 선택했던 기본 템플릿 배경색도 숨긴다.
  // 사진이 캔버스를 모두 덮지 못하는 경우에는 중립적인 흰색만 보인다.
  ctx.fillStyle=img?'#ffffff':s.background;
  ctx.fillRect(0,0,w,h);

  let imageBox=null;
  if(img){
    // 사진이 있으면 기본 템플릿 장식(art)은 그리지 않는다.
    const {sx,sy,sw,sh}=imageSourceRect(s,img);
    const dest=imageDestinationRect(s,img,w,h);
    ctx.drawImage(img,sx,sy,sw,sh,dest.x,dest.y,dest.width,dest.height);
    imageBox={...dest,handleX:dest.x+dest.width,handleY:dest.y+dest.height};
  } else if(s.art!=='none') drawArtwork(ctx,s.art,w,h);

  const layout=layoutText(ctx,s,w,h), textCenterY=layout.top+layout.height/2;
  ctx.save();
  ctx.fillStyle=s.color;
  ctx.textAlign=s.align;
  ctx.textBaseline='middle';
  ctx.font=`700 ${layout.size}px ${fontStack(s.fontFamily)}`;
  if(s.shadow){ctx.shadowColor='#21153055';ctx.shadowBlur=8;ctx.shadowOffsetY=3;}
  ctx.translate(layout.anchor,textCenterY);
  ctx.rotate(s.rotation*Math.PI/180);
  layout.lines.forEach((line,i)=>ctx.fillText(line,0,layout.top+(i+.5)*layout.lineHeight-textCenterY));
  ctx.restore();
  return {...layout,textBox:{x:layout.left,y:layout.top,width:layout.maxWidth,height:layout.height,centerX:layout.anchor,centerY:textCenterY},imageBox};
}
...
export function drawArtwork(c,art,w,h){
  ...
  c.fillStyle=p[0];c.beginPath();c.arc(w*.13,h*.1,w*.43,0,Math.PI*2);c.fill();
  c.fillStyle=p[1];c.beginPath();c.ellipse(w*.98,h*.95,w*.65,h*.38,-.3,0,Math.PI*2);c.fill();
  c.strokeStyle=p[2];c.lineWidth=3;c.beginPath();c.ellipse(w*.67,h*.14,w*.33,h*.25,.4,0,Math.PI*2);c.stroke();
  c.fillStyle=p[2];c.beginPath();c.arc(w*.84,h*.23,11,0,Math.PI*2);c.fill();
  c.font='15px sans-serif';c.fillStyle=art==='lime'?'#414b3388':'#ffffff99';c.textAlign='center';c.fillText('A LITTLE NOTE TO MYSELF',w/2,h-65);
  c.restore();
}
